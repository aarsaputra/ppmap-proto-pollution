...
class ScanConfig(BaseModel):
    ...
    max_workers: int = Field(
        default=3, ge=1, le=50, description="Max concurrent scanning workers"
    )
    verify_ssl: bool = Field(
        default=True, description="Verify SSL certificates during scanning"
    )
    oob_enabled: bool = Field(
        default=False, description="Enable Out-Of-Band (Interact.sh) detection"
    )
    stealth: bool = Field(
        default=False,
    ...
    rate_limit: Optional[int] = Field(
        default=None, description="Requests per minute rate limit"
    )
    delay: float = Field(
