            ...
            while attempt < max_attempts:
                try:
                    ...
                    if attempt >= max_attempts:
                        logger.error(f"Failed {func.__name__} after {max_attempts} attempts: {type(e).__name__} - {e}")
                        raise
                    
                    delay = backoff * (2 ** (attempt - 1)) if backoff_type == 'exponential' else backoff * attempt
                    logger.warning(
