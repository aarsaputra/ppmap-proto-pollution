...
class FalsePositiveEngine:
    ...
    def is_reflected_param(self, response_text: str, param: str, value: str) -> bool:
        ...
        if value_count > 5 and len(value) > 3:
            logger.debug(
                f"Value '{value}' appears {value_count} times - likely reflection"
            )
            return True
    ...
    def secondary_verify(
        self, finding: Dict, driver: Optional[Any] = None
    ) -> VerificationResult:
        """
        ...
        cache_key = (
            f"{finding_type}:{finding.get('url', '')}:{finding.get('payload', '')}"
        )
    ...
    def _perform_verification(
        self, finding: Dict, driver: Optional[Any]
    ) -> VerificationResult:
    ...
    def filter_findings(
        ...
        logger.info(
            f"FP Engine: {len(confirmed)} confirmed, {len(filtered_out)} filtered"
        )

        return confirmed, filtered_out

    def validate_jquery_detection(
        self, version: str, response_text: str
    ) -> VerificationResult:
        """
        ...
        matches = sum(
            1 for p in jquery_patterns if re.search(p, response_text, re.IGNORECASE)
        )
