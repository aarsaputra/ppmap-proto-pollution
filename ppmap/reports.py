...
class EnhancedReportGenerator:
    ...
    def generate_markdown_report(
        self, findings: list, target: str = "", filename: str = None
    ) -> str:
        if not filename:
        ...
        try:
            ...
            lines.append(f"| **Scanner** | PPMAP v4.3.2 Enterprise |")
            ...
            lines.append(f"")
            crit = sum(
                1 for f in findings if "CRITICAL" in str(f.get("severity", "")).upper()
            )
            high = sum(
                1
                for f in findings
                if "HIGH" in str(f.get("severity", "")).upper()
                and "CRITICAL" not in str(f.get("severity", "")).upper()
            )
            med = sum(
                1 for f in findings if "MEDIUM" in str(f.get("severity", "")).upper()
            )
            low = sum(
                1 for f in findings if "LOW" in str(f.get("severity", "")).upper()
            )
            ...
            for current_url, url_findings in grouped_findings.items():
                ...
                lines.append(
                    f'| **Verified** | {"✅ Yes" if verified else "⚠️ Version-based"} |'
                )
                lines.append(f"")
            ...
            lines.append(f"")
            lines.append(
                f"1. **Validate Manually** — Always verify findings in browser Console (F12) with the PoC scripts above"
            )
            lines.append(
                f"2. **Screenshot Evidence** — Take screenshots of Console before & after payload execution"
            )
            lines.append(
                f"3. **Test on Staging** — Never run exploitation payloads on production without authorization"
            )
            lines.append(
                f"4. **Track Remediations** — Document which fixes were applied and verify after deployment"
            )
            lines.append(f"")
            lines.append(f"---")
            lines.append(f"")
            lines.append(
                f"> ⚠️ **Disclaimer:** This report was generated by PPMAP for **authorized security testing only**."
            )
...
<pre><button class="copy-btn" onclick="navigator.clipboard.writeText(this.nextElementSibling.textContent)">Copy</button><code>{esc_script}</code></pre></div>\n"""
    ...
    def generate_all_formats(
        self, findings: list, target: str = "", formats: list = None
    ) -> dict:
        ...
        if "html" in formats:
            html = self.generate_html_report(
                findings, target, f"report_{timestamp}.html"
            )
            if html:
                generated["html"] = html
